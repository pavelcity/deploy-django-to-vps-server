...
git clone gitLink
```
например мой стартовый шаблон блога django с админкой
```
git clone https://github.com/pavelcity/lesson_django.git
```
...
!!! note "выпуск ssl"
    ...
    3. `выпустить ssl сертификат` - по настройке nginx будет предложено выпустить ssl для домена с www  и без www
